...
\begin{titlepage}
  ...
  \begin{center}
    ...
    E.Diociaiuti (LNF-INFN), M.Mackenzie (Northwestern), P.Murat(FNAL)
...
%
    ...
    the RMC background to the search for the \MuMinusEPlus\ conversion process.
    ...
    Using a $\chi^2$ fit results in biased estimates of the RMC photon
...
$90.1 \pm 1.8$ MeV \cite{RMC_1999_PhysRevC.59.2853}, so variations of the endpoint
...
However, given the published 1.8 MeV uncertainty on the RMC spectrum endpoint the two 
numbers are just about 1.5 standard deviations apart, so understanding of the
...
$$
   {dN \over dx} = {e^2 \over \pi} {\kmax^2 \over {m_\mu^2}} (1-2x+2x^2) x (1-x)^2
$$
where $x = k/\kmax$.

We use published parameterizations of the TRIUMF RMC spectrometer response
to convolve them with the closure approximation spectra, fit the experimental data,
...
the published results - around 90 MeV. For background studies at Mu2e, the suggested plan is to use
